...
function logo(s){const initials=s.name.replace(/^(CTP|CINDEA|IPEC)\\s+(de\\s+)?/i,'').trim().split(/\\s+/).filter(Boolean).map(x=>x[0]).join('').slice(0,2).toUpperCase()||'CE';return s.logo ? `<img class="school-logo" src="${s.logo}" alt="Escudo de ${esc(s.name)}" loading="lazy">` : `<span class="school-initial" aria-hidden="true">${esc(initials)}</span>`;}
...
function renderResults(){
 ...
 $('#results').innerHTML=matches.length?matches.slice(0,limit).map(r=>`<article class="offer"><small>${esc(areaById[r.area].short)}</small><h3>${esc(r.name)}</h3><p class="schoolname">${esc(r.school.name)}</p><p class="period">${esc(r.school.period)}</p><div class="offer-bottom"><span class="pill ${r.mode==='Nocturna'?'night':''}">${r.mode==='Nocturna'?'☾':r.mode==='General'?'◆':'☀'} ${r.mode}</span><button class="card-link" data-school="${r.school.id}" aria-label="Ver centro ${esc(r.school.name)} para ${esc(r.name)}, jornada ${r.mode}">Ver centro ↗</button></div></article>`).join(''):'<div class="empty"><h3>No encontramos coincidencias.</h3><p>Pruebe con otra palabra o quite uno de los filtros.</p></div>';
...
function offerColumns(day=[],night=[],general=[]){if(general.length&&!day.length&&!night.length)return `<div class="dialog-columns"><div><h3>◆ Oferta general</h3><ul>${general.map(n=>`<li>${esc(n)}</li>`).join('')}</ul></div></div>`;return `<div class="dialog-columns"><div><h3>☀ Jornada diurna</h3><ul>${day.length?day.map(n=>`<li>${esc(n)}</li>`).join(''):'<li>No indicada en el insumo.</li>'}</ul></div><div><h3>☾ Jornada nocturna</h3><ul>${night.length?night.map(n=>`<li>${esc(n)}</li>`).join(''):'<li>No indicada en el insumo.</li>'}</ul></div></div>`;}
